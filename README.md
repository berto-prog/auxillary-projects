## auxillary-projects
#### onboarding users

## Shell Scripting

Create the project folder called Shell (mkdir Shell)

Move into the Shell folder  (cd Shell)

Create a csv file name names.csv   (touch names.csv)

## Open the names.csv file  (vim names.csv)

### Insert some random names into it. (One name per line)

![image](https://user-images.githubusercontent.com/76639282/120943048-8b87f980-c6fa-11eb-8d58-a2cb76984cdf.png)

In your current home directory change directo .ssh folder ( cd .ssh)


#### create a file for the public key ( touch id_rsa.pub )


#### open the file using your favorite editor and paste in the public key  ( vi id_rsa.pub )


#### create a file for your private key ( touch id_rsa )


#### open the file using your favorite editor and paste in the private key. ( vi id_rsa )


## The Public Key

ssh-rsa 

## The private key

-----BEGIN OPENSSH PRIVATE KEY-----

-----END OPENSSH PRIVATE KEY-----









